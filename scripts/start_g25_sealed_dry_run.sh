...
LOG_DIR="$ROOT/logs/active/g25-sealed-dry-run"
ARCHIVE_DIR="$ROOT/logs/archive/diagnostic/g25_sealed_dry_run"
PID_FILE="$ROOT/.g25_sealed_dry_run.pid"
...
[[ ! -e "$ARCHIVE_DIR" ]] || { echo "Dry-run archive already exists: $ARCHIVE_DIR" >&2; exit 1; }
if [[ -f "$PID_FILE" ]]; then
  pid="$(tr -d '[:space:]' <"$PID_FILE")"
  [[ "$pid" =~ ^[0-9]+$ ]] && kill -0 "$pid" 2>/dev/null && { echo "Dry-run already active" >&2; exit 1; }
  unlink "$PID_FILE"
fi

mkdir -p "$LOG_DIR" "$G25/dry_run/results" "$G25/dry_run/checkpoints"
/usr/bin/python3 "$ROOT/scripts/generate_multi_robot_launch.py" --num-agents 5 --output "$LOG_DIR/runtime_dry_run.launch"
export G25_EVAL_PROFILE=dry_run
export G25_EVAL_MANIFEST="$BASE/datasets/fixed_v1/dense/validation.json.gz"
export G25_EVAL_EPISODES=2 G25_EVAL_SEEDS=20260818
export G25_EVAL_LOG_DIR="$LOG_DIR" G25_EVAL_ARCHIVE_DIR="$ARCHIVE_DIR"
export G25_EVAL_RESULT_DIR="$G25/dry_run/results" G25_EVAL_STATE_DIR="$G25/dry_run/checkpoints"
export G25_EVAL_PID_FILE="$PID_FILE" G25_EVAL_ROS_PORT=18223 G25_EVAL_GAZEBO_PORT=18323
setsid bash "$ROOT/scripts/run_g25_frozen_evaluation_worker.sh" >>"$LOG_DIR/runner.log" 2>&1 < /dev/null &
echo $! >"$PID_FILE"
echo "G25 sealed dry-run started: seven methods x two validation scenes"
