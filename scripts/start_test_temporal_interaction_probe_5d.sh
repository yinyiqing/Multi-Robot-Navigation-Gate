...
RUN_NAME="temporal_interaction_highres_probe_5d_s20260721"
NUM_AGENTS=5
TARGET_EPISODES=30
ROS_PORT="${DRL_MULTI_TEST_ROS_PORT:-12623}"
GAZEBO_PORT="${DRL_MULTI_TEST_GAZEBO_PORT:-12723}"
PID_FILE="$PROJECT_ROOT/.test_temporal_interaction_probe_5d.pid"
LAUNCHFILE="multi_robot_scenario_temporal_interaction_probe_5d_${NUM_AGENTS}.launch"
MANIFEST_PATH="$VIEW_DIR/sensor_probe.json.gz"
...
if [[ -f "$PID_FILE" ]]; then
  ...
  if [[ -n "$old_pid" ]] && kill -0 "$old_pid" 2>/dev/null; then
    echo "Temporal interaction probe is already running with PID $old_pid"
    exit 1
  fi
fi
...
for output in "$TD3_DIR/${STATE_PATH#./}" "$TD3_DIR/${STATS_PATH#./}" "$TRAJECTORY_PATH"; do
  if [[ -e "$output" ]]; then
    echo "Refusing to resume or overwrite existing temporal probe output: $output"
    exit 1
  fi
done
...
setsid bash -lc "
  ...
  export DRL_MULTI_TEMPORAL_LIDAR_DIM=180
...
echo $! > "$PID_FILE"
echo "Started temporal-interaction 5D data probe."
