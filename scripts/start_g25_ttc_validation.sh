...
verify_sha "$RUN_DIR/ttc_rule/selection.json" "9bfb0a1a282dbd5f415c70326369bd54a3ebd6908a9db9fc371d62f5befa481a"

if [[ -f "$PID_FILE" ]]; then
  pid="$(tr -d '[:space:]' <"$PID_FILE")"
  [[ "$pid" =~ ^[0-9]+$ ]] && kill -0 "$pid" 2>/dev/null && {
    echo "G25 TTC validation already running as PID $pid" >&2; exit 1
  }
  unlink "$PID_FILE"
fi
...
/usr/bin/python3 "$ROOT/scripts/generate_multi_robot_launch.py" --num-agents 5 \
  --output "$LOG_DIR/runtime_g25_ttc_validation.launch"
setsid bash "$ROOT/scripts/run_g25_ttc_validation_worker.sh" >>"$LOG_DIR/runner.log" 2>&1 < /dev/null &
echo $! >"$PID_FILE"
echo "G25 TTC validation queued (CPU-only, one Dense256 run)."
echo "PID: $(cat "$PID_FILE")"
echo "It will wait for the existing V4/V5 queue to release the Gazebo lock."
