...
if [[ "${DRL_MULTI_DRY_RUN:-0}" == 1 ]]; then
  echo "Current-generalist N5 efficiency E2 admission dry run passed."
  echo "Candidate: current_generalist_n5_efficiency_e2_s20260810_best"
  echo "Episodes: 120"
  exit 0
fi
...
if [[ -f "$PID_FILE" ]]; then
  ...
  if [[ "$old_pid" =~ ^[0-9]+$ ]] && kill -0 "$old_pid" 2>/dev/null; then
    echo "Current-generalist N5 efficiency E2 admission is already running with PID $old_pid" >&2
    exit 1
  fi
  unlink "$PID_FILE"
fi
...
/usr/bin/python3 "$PROJECT_ROOT/scripts/generate_multi_robot_launch.py" \
  --num-agents 5 --output "$LOG_DIR/runtime_current_generalist_n5_efficiency_e2_admission.launch"
setsid bash "$PROJECT_ROOT/scripts/run_current_generalist_n5_efficiency_e2_admission_worker.sh" \
  >>"$RUNNER_LOG" 2>&1 < /dev/null &
echo $! > "$PID_FILE"

echo "Started current-generalist N5 efficiency E2 paired admission."
echo "PID: $(cat "$PID_FILE")"
echo "Candidate: E2 best; 120 episodes"
