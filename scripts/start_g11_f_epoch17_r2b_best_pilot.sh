...
declare -A EXPECTED=(
  ...
  ["$ROOT/TD3/pytorch_models/capacity_wide_r2b_5a_recipe_n5_seed20260823_best_actor.pth"]="da28dd5820d09845eea07cb68da45a7afd262fe56e8a71f80bf6b5781551523a"
...
if [[ -f "$PID_FILE" ]]; then
  ...
  [[ "$pid" =~ ^[0-9]+$ ]] && kill -0 "$pid" 2>/dev/null && {
    echo "G11-F R2B-best pilot already runs as PID $pid" >&2; exit 1
  }
  unlink "$PID_FILE"
fi
...
[[ ! -e "$ROOT/logs/archive/validation/g11_f_epoch17_gate_r2b_best_pilot" ]] || {
  echo "R2B-best pilot archive already exists" >&2; exit 1
}
mkdir -p "$LOG_DIR"
setsid bash "$ROOT/scripts/run_g11_f_epoch17_r2b_best_pilot_worker.sh" >>"$LOG_DIR/runner.log" 2>&1 < /dev/null &
echo $! >"$PID_FILE"
echo "Started G11-F R2B-best fair capacity extension"
echo "PID: $(cat "$PID_FILE")"
echo "Runs: R2B-best x 2 repeats = 100 episodes"
