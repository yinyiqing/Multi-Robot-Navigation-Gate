...
NUM_AGENTS=5
LAUNCHFILE="multi_robot_scenario_stage2_to_5a_shared_guarded_${NUM_AGENTS}.launch"
LAUNCH_PATH="$TD3_DIR/assets/$LAUNCHFILE"
MODEL_NAME="${DRL_MULTI_TRAIN_FILE_NAME:-TD3_velodyne_multi_v4_curriculum_stage2_to_5a_shared_from_3d2_guarded}"
LOAD_MODEL_NAME="${DRL_MULTI_LOAD_MODEL_NAME:-TD3_velodyne_multi_v4_curriculum_stage2_to_3d2_geo_critic_from_3a_guarded_best}"
ROS_PORT="${DRL_MULTI_ROS_PORT:-11385}"
GAZEBO_PORT="${DRL_MULTI_GAZEBO_PORT:-11485}"

mkdir -p "$LOG_DIR"

timestamp="$(date +%Y%m%d_%H%M%S)"
log_file="$LOG_DIR/train_multi_stage2_to_5a_shared_guarded_detached_${timestamp}.log"

if [[ -f "$PID_FILE" ]]; then
  old_pid="$(cat "$PID_FILE" 2>/dev/null || true)"
  if [[ -n "$old_pid" ]] && kill -0 "$old_pid" 2>/dev/null; then
    echo "A detached stage2-to-5A guarded training process is already running with PID $old_pid"
    exit 1
  fi
fi
...
setsid bash -lc "
  ...
  export DRL_MULTI_TRAINING_VERSION='multi-agent-curriculum-stage2-to-5a-shared-from-3d2-guarded-v1'
...
echo "Detached stage2-to-5A guarded training started."
echo "PID: $(cat "$PID_FILE")"
echo "Agents: $NUM_AGENTS"
echo "Model: $MODEL_NAME"
echo "Warm start actor: $LOAD_MODEL_NAME"
...
echo "Actor LR: ${DRL_MULTI_ACTOR_LR:-0.000002}"
...
echo "Max epochs: ${DRL_MULTI_MAX_EPOCHS:-6}"
