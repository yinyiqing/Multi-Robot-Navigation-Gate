...
def main():
    ...
    pairing = {
        "n5_vs_5a": {
            dimension: {
                name: paired(subset(runs["n5_20k"], ids), subset(runs["5a"], ids))
                for name, ids in groups.items()
            }
            for dimension, groups in dimensions.items()
        },
        "n5_vs_r2_10k": {
            dimension: {
                name: paired(subset(runs["n5_20k"], ids), subset(runs["r2_10k"], ids))
                for name, ids in groups.items()
            }
            for dimension, groups in dimensions.items()
        },
