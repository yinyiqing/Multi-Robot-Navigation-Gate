...
PID_FILE="$PROJECT_ROOT/.test_multi_baseline_3_best_detached.pid"
NUM_AGENTS=3
...
log_file="$LOG_DIR/test_multi_baseline_3_best_detached_${timestamp}.log"

if [[ -f "$PID_FILE" ]]; then
  old_pid="$(cat "$PID_FILE" 2>/dev/null || true)"
  if [[ -n "$old_pid" ]] && kill -0 "$old_pid" 2>/dev/null; then
    echo "A detached 3-agent shared-policy baseline best-model test process is already running with PID $old_pid"
    exit 1
  fi
fi
...
if [[ ! -f "$TD3_DIR/pytorch_models/${MODEL_NAME}_actor.pth" ]]; then
  ...
  echo "Run 3-agent shared-policy baseline training until at least one eval updates the best checkpoint."
  exit 1
fi
...
echo "Detached 3-agent shared-policy baseline best-model test started."
