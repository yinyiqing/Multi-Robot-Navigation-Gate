...
PID_FILE="$PROJECT_ROOT/.train_multi_weighted09_5_detached.pid"
NUM_AGENTS=5
LAUNCHFILE="multi_robot_scenario_weighted09_${NUM_AGENTS}.launch"
LAUNCH_PATH="$TD3_DIR/assets/$LAUNCHFILE"
MODEL_NAME="TD3_velodyne_multi_v4_weighted09_5"
ROS_PORT="11365"
GAZEBO_PORT="11405"
...
log_file="$LOG_DIR/train_multi_weighted09_5_detached_${timestamp}.log"

if [[ -f "$PID_FILE" ]]; then
  old_pid="$(cat "$PID_FILE" 2>/dev/null || true)"
  if [[ -n "$old_pid" ]] && kill -0 "$old_pid" 2>/dev/null; then
    echo "A detached 5-agent weighted09 training process is already running with PID $old_pid"
    exit 1
  fi
fi
...
if [[ -n "$existing_pid" ]]; then
  ...
  echo "Please stop the current multi-agent training before starting 5-agent weighted09 detached mode."
  exit 1
fi
...
setsid bash -lc "
  ...
  export DRL_MULTI_REWARD_SELF_WEIGHT=0.9
  ...
  export DRL_MULTI_TRAINING_VERSION='multi-agent-weighted09-5-v1'
...
echo "Detached 5-agent weighted09 training started."
echo "PID: $(cat "$PID_FILE")"
echo "Model: $MODEL_NAME"
echo "Launch: $LAUNCH_PATH"
echo "Warm start: TD3_velodyne_multi_v4"
echo "Reward: distance-weighted 0.9 own + 0.1 visible-neighbor"
