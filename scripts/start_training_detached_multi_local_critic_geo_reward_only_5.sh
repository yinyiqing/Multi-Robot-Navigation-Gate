...
MODEL_NAME="TD3_velodyne_multi_v4_local_critic_geo_reward_only_5"
ROS_PORT="11366"
GAZEBO_PORT="11406"
...
log_file="$LOG_DIR/train_multi_local_critic_geo_reward_only_5_detached_${timestamp}.log"

if [[ -f "$PID_FILE" ]]; then
  old_pid="$(cat "$PID_FILE" 2>/dev/null || true)"
  if [[ -n "$old_pid" ]] && kill -0 "$old_pid" 2>/dev/null; then
    echo "A detached 5-agent reward-only geometry local-critic training process is already running with PID $old_pid"
    exit 1
  fi
fi
...
if [[ -n "$existing_pid" ]]; then
  ...
  echo "Please stop the current multi-agent training before starting 5-agent reward-only geometry local-critic detached mode."
  exit 1
fi
...
setsid bash -lc "
  ...
  export DRL_MULTI_TRAINING_VERSION='multi-agent-local-neighborhood-critic-geo-reward-only-5-v1'
...
echo "Detached 5-agent reward-only geometry local-critic training started."
