...
PID_FILE="$PROJECT_ROOT/.test_multi_local_critic_best_detached.pid"
LAUNCHFILE="multi_robot_scenario_multi_2.launch"
MODEL_NAME="TD3_velodyne_multi_v4_local_critic_best"
ROS_PORT="11343"
GAZEBO_PORT="11383"
...
log_file="$LOG_DIR/test_multi_local_critic_best_detached_${timestamp}.log"

if [[ -f "$PID_FILE" ]]; then
  old_pid="$(cat "$PID_FILE" 2>/dev/null || true)"
  if [[ -n "$old_pid" ]] && kill -0 "$old_pid" 2>/dev/null; then
    echo "A detached local-critic best-model test process is already running with PID $old_pid"
    exit 1
  fi
fi
...
if [[ ! -f "$TD3_DIR/pytorch_models/${MODEL_NAME}_actor.pth" ]]; then
  ...
  echo "Run local-critic training until at least one eval updates the best checkpoint."
  exit 1
fi
...
echo "Detached local-critic best-model test started."
