...
RUN_DIR="$PROJECT_ROOT/experiments/03_保留专门化/02_论文主线/13_当前场景普通Actor重训/local_data/n5_efficiency_e2_admission"
RESULTS_DIR="$RUN_DIR/results"
CHECKPOINT_DIR="$RUN_DIR/checkpoints"
LOG_DIR="$PROJECT_ROOT/logs/active/current-generalist-r2style/n5-efficiency-e2-admission"
LAUNCHFILE="$LOG_DIR/runtime_current_generalist_n5_efficiency_e2_admission.launch"
PID_FILE="$PROJECT_ROOT/.current_generalist_n5_efficiency_e2_admission.pid"
...
N5_ADMISSION_DIR="$PROJECT_ROOT/experiments/03_保留专门化/02_论文主线/13_当前场景普通Actor重训/local_data/n5_admission/results"
ROS_PORT=15452
GAZEBO_PORT=15552
EPISODES=120
SEED=20260817
...
run_e2() {
  local run_name="current_generalist_n5_efficiency_e2_admission_s${SEED}"
  ...
  for attempt in $(seq 1 5); do
    ...
    echo "Starting $run_name attempt $attempt with Actor $E2_MODEL"
    wait_for_ports
    set +e
...
/usr/bin/python3 "$PROJECT_ROOT/scripts/analyze_current_generalist_n5_efficiency_e2_admission.py" \
  --manifest "$MANIFEST" \
  ...
  --n5-result "$N5_ADMISSION_DIR/current_generalist_n5_admission_n5_s20260817.npy" \
  --e2-result "$RESULTS_DIR/current_generalist_n5_efficiency_e2_admission_s${SEED}.npy" \
  --output "$RUN_DIR/summary.json" \
  --seed "$SEED" >"$LOG_DIR/analysis.log" 2>&1
echo "Current-generalist N5 efficiency E2 paired admission complete."
