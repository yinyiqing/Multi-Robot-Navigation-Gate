...
if [[ "${DRL_MULTI_DRY_RUN:-0}" == 1 ]]; then
  echo "Current-generalist E2 + epoch16 oracle admission dry run passed."
...
if [[ -f "$PID_FILE" ]]; then
  ...
  if [[ "$old_pid" =~ ^[0-9]+$ ]] && kill -0 "$old_pid" 2>/dev/null; then
    echo "Current-generalist E2 oracle admission is already running with PID $old_pid" >&2
    exit 1
  fi
  unlink "$PID_FILE"
fi
...
/usr/bin/python3 "$PROJECT_ROOT/scripts/generate_multi_robot_launch.py" \
  --num-agents 5 --output "$LOG_DIR/runtime_current_generalist_e2_oracle_epoch16_admission.launch"
setsid bash "$PROJECT_ROOT/scripts/run_current_generalist_e2_oracle_admission_worker.sh" \
  >>"$RUNNER_LOG" 2>&1 < /dev/null &
echo $! > "$PID_FILE"

echo "Started current-generalist E2 + epoch16 oracle admission."
echo "PID: $(cat "$PID_FILE")"
echo "Oracle: E2 normally, epoch16 when nearest active robot <= 2.0 m"
