...
PROTOCOL="$G33/local_data/protocol_round2"
COLLECTION="$G33/local_data/counterfactual_round2"
LOG="$G33/logs/data_queue_round2"
OLD_PROTOCOL="$G33/local_data/protocol"

mkdir -p "$PROTOCOL" "$COLLECTION" "$LOG"
exec > >(tee -a "$LOG/queue.log") 2>&1
echo "[$(date -Is)] G33 round2 queue started"
...
python3 "$ROOT/scripts/select_g27_pilot_anchors.py" \
  ...
  --minimum-step 4 --maximum-step 40 --per-cell 64 --matching \
  --exclude-selection "$OLD_PROTOCOL/train_anchors.json" \
  --protocol "G33-quality-collection-train-round2-v1"
fi
...
python3 "$ROOT/scripts/select_g27_pilot_anchors.py" \
  ...
  --protocol "G33-quality-collection-validation-round2-v1"
fi
...
python3 -u "$ROOT/scripts/run_g27_counterfactual_pilot.py" \
  --profile collect --anchors "$PROTOCOL/train_anchors.json" \
  --manifest "$VIEW/train.json.gz" --navigation-actor "$NAVIGATION" \
  --interaction-actor "$INTERACTION" --detector-checkpoint "$DETECTOR" \
  --output-dir "$COLLECTION/train" --jobs "${G33_JOBS:-4}" \
  --base-ros-port 18710 --base-gazebo-port 19710 --seed 20260913 --resume

python3 -u "$ROOT/scripts/run_g27_counterfactual_pilot.py" \
  --profile collect --anchors "$PROTOCOL/validation_anchors.json" \
  --manifest "$VIEW/validation.json.gz" --navigation-actor "$NAVIGATION" \
  --interaction-actor "$INTERACTION" --detector-checkpoint "$DETECTOR" \
  --output-dir "$COLLECTION/validation" --jobs "${G33_JOBS:-4}" \
  --base-ros-port 18810 --base-gazebo-port 19810 --seed 20260913 --resume
...
echo "[$(date -Is)] G33 round2 queue completed; no training or closed-loop evaluation was started"
