...
MODEL_NAME="TD3_velodyne_multi_v4_curriculum_stage2_2d_local_critic_from_2a"
...
log_file="$LOG_DIR/train_multi_stage2_2d_local_critic_from_2a_detached_${timestamp}.log"

if [[ -f "$PID_FILE" ]]; then
  old_pid="$(cat "$PID_FILE" 2>/dev/null || true)"
  if [[ -n "$old_pid" ]] && kill -0 "$old_pid" 2>/dev/null; then
    echo "A detached stage2 2D local-critic training process is already running with PID $old_pid"
    exit 1
  fi
fi
...
setsid bash -lc "
  ...
  export DRL_MULTI_MAX_EPOCHS='${DRL_MULTI_MAX_EPOCHS:-16}'
  export DRL_MULTI_EXPL_NOISE='${DRL_MULTI_EXPL_NOISE:-0.08}'
  export DRL_MULTI_EXPL_MIN='${DRL_MULTI_EXPL_MIN:-0.025}'
  export DRL_MULTI_ACTOR_LR='${DRL_MULTI_ACTOR_LR:-0.00006}'
  export DRL_MULTI_CRITIC_LR='${DRL_MULTI_CRITIC_LR:-0.00008}'
  export DRL_MULTI_TRAINING_VERSION='stage2-2d-local-critic-from-2a-v1'
...
echo "Detached stage2 2D local-critic training started."
...
echo "Local critic: 1"
echo "Max epochs: ${DRL_MULTI_MAX_EPOCHS:-16}"
