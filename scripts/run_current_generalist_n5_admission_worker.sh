...
RUN_DIR="$PROJECT_ROOT/experiments/03_保留专门化/02_论文主线/13_当前场景普通Actor重训/local_data/n5_admission"
RESULTS_DIR="$RUN_DIR/results"
CHECKPOINT_DIR="$RUN_DIR/checkpoints"
LOG_DIR="$PROJECT_ROOT/logs/active/current-generalist-r2style/n5-admission"
LAUNCHFILE="$LOG_DIR/runtime_current_generalist_n5_admission.launch"
PID_FILE="$PROJECT_ROOT/.current_generalist_n5_admission.pid"
LOCK_FILE="/tmp/local_critic_multi_robot_training.lock"
N5_MODEL="current_generalist_n5_original_broad_s20260810_best"
...
run_n5() {
  local run_name="current_generalist_n5_admission_n5_s${SEED}"
  ...
  export DRL_MULTI_STANDARD_ACTOR_FILE="$N5_MODEL"
  ...
  for attempt in $(seq 1 5); do
    ...
    echo "Starting $run_name attempt $attempt with Actor $N5_MODEL"
    wait_for_ports
    set +e
...
/usr/bin/python3 "$PROJECT_ROOT/scripts/analyze_current_generalist_n5_admission.py" \
  --manifest "$MANIFEST" \
  --n5-result "$RESULTS_DIR/current_generalist_n5_admission_n5_s${SEED}.npy" \
...
echo "Current-generalist N5 paired admission complete."
