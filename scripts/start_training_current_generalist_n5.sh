...
MODEL_NAME="${DRL_MULTI_TRAIN_FILE_NAME:-current_generalist_n5_original_broad_s20260810}"
LOAD_MODEL="${DRL_MULTI_LOAD_MODEL_NAME:-current_generalist_n3_original_broad_s20260810_best}"
SAFE_MODEL="${MODEL_NAME//[^A-Za-z0-9_]/_}"
PID_FILE="${DRL_MULTI_PID_FILE:-$PROJECT_ROOT/.train_${SAFE_MODEL}.pid}"
LOG_DIR="${DRL_MULTI_LOG_DIR:-$PROJECT_ROOT/logs/active/current-generalist-r2style/n5}"
LOCK_FILE="/tmp/local_critic_multi_robot_training.lock"
LAUNCHFILE="$LOG_DIR/runtime_current_generalist_n5.launch"
ROS_PORT="${DRL_MULTI_ROS_PORT:-14851}"
GAZEBO_PORT="${DRL_MULTI_GAZEBO_PORT:-14951}"
SEED="${DRL_MULTI_SEED:-20260816}"
...
declare -A EXPECTED_SHA256=(
  ...
  ["$TD3_DIR/pytorch_models/${LOAD_MODEL}_actor.pth"]="e35eb07cadff85dc29bf9f470ea7df91e9a5fc34e8bcb99c1bdea2c70b15fdcd"
  ["$TD3_DIR/pytorch_models/${LOAD_MODEL}_critic.pth"]="c044abb91d514a552207168c3031f01dcfd2afb3f1d6200599b4e951af19c629"
)
for path in "${!EXPECTED_SHA256[@]}"; do
  [[ -f "$path" ]] || { echo "Required N5 input is missing: $path" >&2; exit 1; }
  actual="$(sha256sum "$path" | awk '{print $1}')"
  [[ "$actual" == "${EXPECTED_SHA256[$path]}" ]] || {
    echo "N5 input hash mismatch: $path" >&2
...
if [[ "${DRL_MULTI_DRY_RUN:-0}" == 1 ]]; then
  cat <<EOF
Current-generalist N5 dry run passed.
Model: $MODEL_NAME
...
if [[ -f "$PID_FILE" ]]; then
  ...
  if [[ "$old_pid" =~ ^[0-9]+$ ]] && kill -0 "$old_pid" 2>/dev/null; then
    echo "Current-generalist N5 is already running with PID $old_pid" >&2
    exit 1
  fi
  unlink "$PID_FILE"
fi
for artifact in \
  ...
  [[ ! -e "$artifact" ]] || { echo "Fresh N5 output already exists: $artifact" >&2; exit 1; }
done
...
if pgrep -af '(^|/)(gzserver|rosmaster)( |$)' >/dev/null; then
  echo "An existing Gazebo or ROS master is running; N5 will not start a second simulator" >&2
  exit 1
fi
...
setsid bash -lc "
  ...
  export DRL_MULTI_TRAINING_VERSION=current-generalist-r2style-n5-original-from-n3-v1
...
echo $! > "$PID_FILE"
echo "Started current-generalist N5."
