...
log_file="$LOG_DIR/test_multi_dense_baseline_5_best_detached_${timestamp}.log"

if [[ -f "$PID_FILE" ]]; then
  old_pid="$(cat "$PID_FILE" 2>/dev/null || true)"
  if [[ -n "$old_pid" ]] && kill -0 "$old_pid" 2>/dev/null; then
    echo "A detached dense 5-agent baseline best-model test process is already running with PID $old_pid"
    exit 1
  fi
fi
...
echo "Detached dense 5-agent baseline best-model test started."
