...
MANIFEST="${G27_P4_MANIFEST:?G27_P4_MANIFEST is required}"
AMENDMENT="${G27_P4_AMENDMENT:?G27_P4_AMENDMENT is required}"
EPISODES="${G27_P4_EPISODES:?G27_P4_EPISODES is required}"
SEEDS_TEXT="${G27_P4_SEEDS:?G27_P4_SEEDS is required}"
LOG_DIR="${G27_P4_LOG_DIR:?G27_P4_LOG_DIR is required}"
RESULT_DIR="${G27_P4_RESULT_DIR:?G27_P4_RESULT_DIR is required}"
STATE_DIR="${G27_P4_STATE_DIR:?G27_P4_STATE_DIR is required}"
ARCHIVE_DIR="${G27_P4_ARCHIVE_DIR:?G27_P4_ARCHIVE_DIR is required}"
PID_FILE="${G27_P4_PID_FILE:?G27_P4_PID_FILE is required}"
LAUNCHFILE="${G27_P4_LAUNCHFILE:?G27_P4_LAUNCHFILE is required}"
ROS_PORT="${G27_P4_ROS_PORT:?G27_P4_ROS_PORT is required}"
GAZEBO_PORT="${G27_P4_GAZEBO_PORT:?G27_P4_GAZEBO_PORT is required}"
...
verify_partial_result() {
  python3 - "$1" "$2" "$EPISODES" "$MANIFEST" <<'PY'
...
if [str(item) for item in rows[:,12]] != expected:
    raise SystemExit('partial scenario order mismatch')
...
clear_policy_environment() {
  unset DRL_MULTI_STANDARD_ACTOR_FILE DRL_MULTI_DENSE_ACTOR_FILE
  unset DRL_MULTI_ACTOR_SELECTION_MODE DRL_MULTI_GATE_DETECTOR_CHECKPOINT
  ...
  case "$method" in
    ...
    b2|b3)
      ...
      if [[ "$method" == "b2" ]]; then
        ...
        export DRL_MULTI_GATE_CHECKPOINT="$B3"
      fi
    ...
    *) echo "Unknown P4 method: $method" >&2; return 2 ;;
  esac
}
...
run_one() {
  ...
  run_name="g27_p4_${method}_s${seed}"
  ...
  for attempt in $(seq 1 10); do
    ...
    echo "$run_name interrupted at $progress/$EPISODES (exit=$status); resuming exact run"
  done
...
for seed_index in "${!SEEDS[@]}"; do
  ...
  for offset in 0 1 2; do
    ordered+=("${METHODS[$(((seed_index + offset) % 3))]}")
  done
...
python3 - "$MANIFEST" "$AMENDMENT" "$EPISODES" "$SEEDS_TEXT" "$RESULT_DIR" <<'PY'
import hashlib,json,sys
from pathlib import Path

manifest,amendment,episodes,seeds_text,result_dir=sys.argv[1:]
...
for method in methods:
    for seed in seeds:
        ...
        if not path.is_file(): raise SystemExit('missing completed result: %s' % path)
...
record={
    ...
    'continuation_amendment_sha256':hashlib.sha256(Path(amendment).read_bytes()).hexdigest(),
    'episodes_per_method':episodes,
    'seeds':seeds,
    'methods':methods,
    'total_episodes':episodes*len(seeds)*len(methods),
    'result_sha256':hashes,
    'sealed_test_read':True,
    'actor_or_router_updated':False,
}
output=result_dir.parent/'p4_completion.json'
output.write_text(json.dumps(record,indent=2)+'\n',encoding='utf-8')
print(output)
PY

set +e
python3 "$ROOT/scripts/analyze_g27_p4_test.py"
analysis_status=$?
set -e
...
echo "G27 P4 complete: 3 methods x ${EPISODES} scenes x ${#SEEDS[@]} repeats"
