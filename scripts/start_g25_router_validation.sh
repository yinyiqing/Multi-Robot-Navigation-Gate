...
verify_sha "$RUN_DIR/router_ablations/v4_single_frame/any/T1/best.pt" "4189e5976b0de06ead49ba66ff9b11f7cf2ef029a8f39709f8a56cc72d4a858a"
verify_sha "$RUN_DIR/router_ablations/v5_no_action_difference/any/T1/best.pt" "95ba582ec2c9a35ed60f17a21936c83166267d3b5ec25b302e6d7bfad3c94012"

if [[ -f "$PID_FILE" ]]; then
  pid="$(tr -d '[:space:]' <"$PID_FILE")"
  [[ "$pid" =~ ^[0-9]+$ ]] && kill -0 "$pid" 2>/dev/null && {
    echo "G25 Router validation already running as PID $pid" >&2; exit 1
  }
  unlink "$PID_FILE"
fi
...
/usr/bin/python3 "$ROOT/scripts/generate_multi_robot_launch.py" --num-agents 5 \
  --output "$LOG_DIR/runtime_g25_router_validation.launch"
setsid bash "$ROOT/scripts/run_g25_router_validation_worker.sh" >>"$LOG_DIR/runner.log" 2>&1 < /dev/null &
echo $! >"$PID_FILE"
echo "G25 Router validation queued (CPU-only, serial)."
echo "PID: $(cat "$PID_FILE")"
echo "Runs: V4 single-frame, then V5 no-action-difference"
