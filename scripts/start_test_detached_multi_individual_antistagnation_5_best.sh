...
PID_FILE="$PROJECT_ROOT/.test_multi_individual_antistagnation_5_best_detached.pid"
NUM_AGENTS=5
LAUNCHFILE="multi_robot_scenario_individual_antistagnation_${NUM_AGENTS}.launch"
LAUNCH_PATH="$TD3_DIR/assets/$LAUNCHFILE"
MODEL_NAME="TD3_velodyne_multi_v4_individual_antistagnation_5_best"
...
log_file="$LOG_DIR/test_multi_individual_antistagnation_5_best_detached_${timestamp}.log"

if [[ -f "$PID_FILE" ]]; then
  old_pid="$(cat "$PID_FILE" 2>/dev/null || true)"
  if [[ -n "$old_pid" ]] && kill -0 "$old_pid" 2>/dev/null; then
    echo "A detached 5-agent individual-antistagnation best-model test process is already running with PID $old_pid"
    exit 1
  fi
fi
...
if [[ ! -f "$TD3_DIR/pytorch_models/${MODEL_NAME}_actor.pth" ]]; then
  ...
  echo "Run 5-agent individual-antistagnation training until at least one eval updates the best checkpoint."
  exit 1
fi
...
echo "Detached 5-agent individual-antistagnation best-model test started."
