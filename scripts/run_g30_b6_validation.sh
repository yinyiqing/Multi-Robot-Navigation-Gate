...
CHECKPOINT="$G30/local_data/training/seed20260911/best_runtime.pt"
SUMMARY="$G30/local_data/training/seed20260911/summary.json"
RESULT_DIR="$G30/local_data/validation/results"
STATE_DIR="$G30/local_data/validation/checkpoints"
LOG_DIR="$G30/logs/validation"
PID_FILE="$ROOT/.g30_b6_validation.pid"
LAUNCHFILE="$LOG_DIR/runtime_g30_b6.launch"
METHOD="b6"
SEED="20260911"
TARGET_EPISODES="${G30_TARGET_EPISODES:-256}"
RESULT_TAG="${G30_RESULT_TAG:-g30_dense256_b6}"
if [[ "$METHOD" == "b2" ]]; then
  ...
  [[ "$METHOD" == "b4" || "$METHOD" == "b5" || "$METHOD" == "b6" ]] || { echo "Unknown reward-aware method: $METHOD" >&2; exit 2; }
fi
LOG_PREFIX="${METHOD}"
...
[[ -f "$CHECKPOINT" ]] || { echo "G30 B6 checkpoint is missing" >&2; exit 2; }
...
if summary.get("checkpoint", {}).get("sha256") != actual:
    raise SystemExit("G30 checkpoint/summary hash mismatch")
PY
fi
...
if [[ -f "$PID_FILE" ]]; then
  ...
  [[ "$old_pid" =~ ^[0-9]+$ ]] && kill -0 "$old_pid" 2>/dev/null && {
    echo "G30 B6 validation already running as PID $old_pid" >&2; exit 1;
  }
  unlink "$PID_FILE"
fi
...
if [[ "$TARGET_EPISODES" == "256" ]] && verify_result 2>/dev/null; then
  echo "G30 B6 validation already complete; reusing it"
else
  ...
  export DRL_MULTI_SEED="$SEED" DRL_MULTI_TEST_FILE_NAME="g30_dense256_${METHOD}_s${SEED}"
  ...
  for attempt in $(seq 1 10); do
    echo "Starting G30 B6 validation attempt $attempt"
    set +e
  ...
  if verify_result 2>/dev/null; then
      echo "G30 B6 validation completed"
      break
    fi
    if [[ ! -f "$RESULT" || ! -f "$STATE" ]]; then
      echo "G30 validation failed before resumable state (exit=$status)" >&2; exit 1
    fi
    progress="$(verify_partial)" || exit 1
    echo "G30 validation interrupted at $progress/256 (exit=$status); resuming exact run"
    [[ "$attempt" != 10 ]] || { echo "G28 validation failed after 10 attempts" >&2; exit 1; }
  done
fi

echo "G30 B6 raw validation complete: $RESULT"
