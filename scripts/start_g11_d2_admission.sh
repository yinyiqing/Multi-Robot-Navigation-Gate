...
RUNNER_LOG="$ACTIVE_LOG_DIR/admission_runner.log"
ROS_PORT=14823
GAZEBO_PORT=14923

declare -A EXPECTED_SHA=(
  ["$PROJECT_ROOT/experiments/03_保留专门化/02_论文主线/datasets/fixed_v1/views/g11_d2_admission_v1/validation.json.gz"]="6250b941f127d550641a621d4253e17ea0770ff3c0cb94e6254e1f26b9f4978a"
  ...
  ["$PROJECT_ROOT/experiments/03_保留专门化/02_论文主线/results/06_Gate开发/D5_G2_interaction_gate_v1/local_data/model/oracle_front_v1/best.pt"]="17e62f3019305945876a97faa4ddb9a3327533ca1d89e0559e6430d52d654de2"
)
...
if [[ -f "$PID_FILE" ]]; then
  ...
  if [[ "$old_pid" =~ ^[0-9]+$ ]] && kill -0 "$old_pid" 2>/dev/null; then
    echo "G11-D2 admission is already running with PID $old_pid" >&2
    exit 1
  fi
  unlink "$PID_FILE"
fi
...
setsid bash "$PROJECT_ROOT/scripts/run_g11_d2_admission_worker.sh" \
  >>"$RUNNER_LOG" 2>&1 < /dev/null &
echo $! > "$PID_FILE"

echo "Started G11-D2 independent 200-scene admission."
echo "PID: $(cat "$PID_FILE")"
echo "Runs: 5A/rule/old-G2A/A1/B2/oracle/epoch16, serial, CPU only"
