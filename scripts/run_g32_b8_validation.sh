...
G30="$BASE/32_B8冻结B2独立RewardHead"
...
CHECKPOINT="$G30/local_data/training/seed20260911_b8/best.pt"
SUMMARY="$G30/local_data/training/seed20260911_b8/summary.json"
RESULT_DIR="$G30/local_data/validation/results"
STATE_DIR="$G30/local_data/validation/checkpoints"
LOG_DIR="$G30/logs/validation"
PID_FILE="$ROOT/.g32_b8_validation.pid"
LAUNCHFILE="$LOG_DIR/runtime_g32_b8.launch"
METHOD="b8"
SEED="20260911"
TARGET_EPISODES="${G30_TARGET_EPISODES:-256}"
RESULT_TAG="${G30_RESULT_TAG:-g32_dense256_b8}"
if [[ "$METHOD" == "b2" ]]; then
  ...
  [[ "$METHOD" == "b4" || "$METHOD" == "b5" || "$METHOD" == "b6" || "$METHOD" == "b7" || "$METHOD" == "b8" ]] || { echo "Unknown reward-aware method: $METHOD" >&2; exit 2; }
fi
LOG_PREFIX="${METHOD}"
...
echo "G32 B8 raw validation complete: $RESULT"
