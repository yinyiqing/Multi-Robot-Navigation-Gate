...
PID_FILE="$PROJECT_ROOT/.train_multi_stage2_2d_local_critic_from_2a_guarded_detached.pid"
LAUNCHFILE="multi_robot_scenario_multi_2.launch"
MODEL_NAME="TD3_velodyne_multi_v4_curriculum_stage2_2d_local_critic_from_2a_guarded"
LOAD_MODEL_NAME="${DRL_MULTI_LOAD_MODEL_NAME:-TD3_velodyne_multi_v4_curriculum_stage2_2a_shared_from_stage1g_best}"
ROS_PORT="${DRL_MULTI_ROS_PORT:-11375}"
GAZEBO_PORT="${DRL_MULTI_GAZEBO_PORT:-11475}"

mkdir -p "$LOG_DIR"

timestamp="$(date +%Y%m%d_%H%M%S)"
log_file="$LOG_DIR/train_multi_stage2_2d_local_critic_from_2a_guarded_detached_${timestamp}.log"

if [[ -f "$PID_FILE" ]]; then
  old_pid="$(cat "$PID_FILE" 2>/dev/null || true)"
  if [[ -n "$old_pid" ]] && kill -0 "$old_pid" 2>/dev/null; then
    echo "A detached guarded stage2 2D local-critic training process is already running with PID $old_pid"
    exit 1
  fi
fi
...
setsid bash -lc "
  ...
  export DRL_MULTI_EXPL_NOISE='${DRL_MULTI_EXPL_NOISE:-0.12}'
  export DRL_MULTI_EXPL_MIN='${DRL_MULTI_EXPL_MIN:-0.03}'
  export DRL_MULTI_ACTOR_LR='${DRL_MULTI_ACTOR_LR:-0.00002}'
  export DRL_MULTI_CRITIC_LR='${DRL_MULTI_CRITIC_LR:-0.00008}'
  export DRL_MULTI_LOCAL_CRITIC_ACTOR_UPDATE_DELAY_STEPS='${DRL_MULTI_LOCAL_CRITIC_ACTOR_UPDATE_DELAY_STEPS:-15000}'
  export DRL_MULTI_TRAINING_VERSION='stage2-2d-local-critic-from-2a-guarded-v1'
...
echo "Detached guarded stage2 2D local-critic training started."
echo "PID: $(cat "$PID_FILE")"
echo "Agents: 2"
echo "Model: $MODEL_NAME"
echo "Actor warm start: $LOAD_MODEL_NAME"
echo "Critic: local neighborhood context, newly initialized"
echo "Actor update delay steps: ${DRL_MULTI_LOCAL_CRITIC_ACTOR_UPDATE_DELAY_STEPS:-15000}"
