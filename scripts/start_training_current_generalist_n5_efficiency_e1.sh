...
MODEL_NAME="${DRL_MULTI_TRAIN_FILE_NAME:-current_generalist_n5_efficiency_e1_s20260810}"
...
LOG_DIR="${DRL_MULTI_LOG_DIR:-$PROJECT_ROOT/logs/active/current-generalist-r2style/n5-efficiency-e1}"
LOCK_FILE="/tmp/local_critic_multi_robot_training.lock"
LAUNCHFILE="$LOG_DIR/runtime_current_generalist_n5_efficiency_e1.launch"
ROS_PORT="${DRL_MULTI_ROS_PORT:-15651}"
GAZEBO_PORT="${DRL_MULTI_GAZEBO_PORT:-15751}"
SEED="${DRL_MULTI_SEED:-20260818}"
...
for path in "${!EXPECTED_SHA256[@]}"; do
  [[ -f "$path" ]] || { echo "Required E1 input is missing: $path" >&2; exit 1; }
  actual="$(sha256sum "$path" | awk '{print $1}')"
  [[ "$actual" == "${EXPECTED_SHA256[$path]}" ]] || {
    echo "E1 input hash mismatch: $path" >&2
...
if [[ "${DRL_MULTI_DRY_RUN:-0}" == 1 ]]; then
  cat <<EOF
Current-generalist N5 efficiency E1 dry run passed.
Model: $MODEL_NAME
...
if [[ -f "$PID_FILE" ]]; then
  ...
  if [[ "$old_pid" =~ ^[0-9]+$ ]] && kill -0 "$old_pid" 2>/dev/null; then
    echo "Current-generalist N5 efficiency E1 is already running with PID $old_pid" >&2
    exit 1
  fi
  unlink "$PID_FILE"
fi
for artifact in \
  ...
  [[ ! -e "$artifact" ]] || { echo "Fresh E1 output already exists: $artifact" >&2; exit 1; }
done
...
if pgrep -af '(^|/)(gzserver|rosmaster)( |$)' >/dev/null; then
  echo "An existing Gazebo or ROS master is running; E1 will not start a second simulator" >&2
  exit 1
fi
...
setsid bash -lc "
  ...
  export DRL_MULTI_TRAINING_VERSION=current-generalist-n5-efficiency-e1-timeout-repair-v1
  ...
  export DRL_MULTI_PROGRESS_REWARD_WEIGHT=25.0
  export DRL_MULTI_FORWARD_REWARD_WEIGHT=0.8
  export DRL_MULTI_TURN_PENALTY_WEIGHT=0.15
  export DRL_MULTI_OBSTACLE_PENALTY_WEIGHT=0.5
  export DRL_MULTI_STAGNATION_PENALTY_WEIGHT=0.03
  export DRL_MULTI_TIMEOUT_REWARD=-120.0
  export DRL_MULTI_USE_SAFE_RECOVERY_REWARD=1
  export DRL_MULTI_SAFE_RECOVERY_PENALTY=0.15
  export DRL_MULTI_SAFE_RECOVERY_LINEAR_THRESHOLD=0.25
  export DRL_MULTI_SAFE_RECOVERY_PROGRESS_THRESHOLD=0.004
  export DRL_MULTI_SAFE_RECOVERY_MIN_LASER=0.55
  export DRL_MULTI_SAFE_RECOVERY_ROBOT_DISTANCE=1.0
  export DRL_MULTI_SAFE_RECOVERY_PROGRESS_BONUS_WEIGHT=0.12
  export DRL_MULTI_SAFE_RECOVERY_IDLE_PENALTY_WEIGHT=0.20
...
echo $! > "$PID_FILE"
echo "Started current-generalist N5 efficiency E1."
