...
RUN_DIR="$PROJECT_ROOT/experiments/03_保留专门化/02_论文主线/13_当前场景普通Actor重训/local_data/e2_oracle_epoch16_admission"
RESULTS_DIR="$RUN_DIR/results"
CHECKPOINT_DIR="$RUN_DIR/checkpoints"
LOG_DIR="$PROJECT_ROOT/logs/active/current-generalist-r2style/e2-oracle-epoch16-admission"
LAUNCHFILE="$LOG_DIR/runtime_current_generalist_e2_oracle_epoch16_admission.launch"
PID_FILE="$PROJECT_ROOT/.current_generalist_e2_oracle_epoch16_admission.pid"
...
run_oracle() {
  local run_name="current_generalist_e2_oracle_epoch16_admission_s${SEED}"
...
/usr/bin/python3 "$PROJECT_ROOT/scripts/analyze_current_generalist_e2_oracle_admission.py" \
  --manifest "$MANIFEST" \
  --five-a-result "$REFERENCE_DIR/g12_r2_n5_admission_5a_s${SEED}.npy" \
  --r2-result "$REFERENCE_DIR/g12_r2_n5_admission_r2_10k_s${SEED}.npy" \
  --n5-result "$N5_DIR/current_generalist_n5_admission_n5_s${SEED}.npy" \
  --e2-result "$E2_DIR/current_generalist_n5_efficiency_e2_admission_s${SEED}.npy" \
  --e2-oracle-result "$RESULTS_DIR/current_generalist_e2_oracle_epoch16_admission_s${SEED}.npy" \
  --output "$RUN_DIR/summary.json" \
  --seed "$SEED" >"$LOG_DIR/analysis.log" 2>&1
echo "Current-generalist E2 + epoch16 oracle paired admission complete."
