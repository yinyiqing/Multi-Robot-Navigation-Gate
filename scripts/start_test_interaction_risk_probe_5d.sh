...
RUN_NAME="interaction_risk_probe_v1_5d_s20260721"
NUM_AGENTS=5
TARGET_EPISODES=60
ROS_PORT="${DRL_MULTI_TEST_ROS_PORT:-12621}"
GAZEBO_PORT="${DRL_MULTI_TEST_GAZEBO_PORT:-12721}"
PID_FILE="$PROJECT_ROOT/.test_interaction_risk_probe_5d.pid"
LAUNCHFILE="multi_robot_scenario_interaction_risk_probe_5d_${NUM_AGENTS}.launch"
...
if [[ -f "$PID_FILE" ]]; then
  ...
  if [[ -n "$old_pid" ]] && kill -0 "$old_pid" 2>/dev/null; then
    echo "Interaction-risk probe is already running with PID $old_pid"
    exit 1
  fi
fi
...
for output in "$TD3_DIR/${STATE_PATH#./}" "$TD3_DIR/${STATS_PATH#./}" "$TRAJECTORY_PATH"; do
  if [[ -e "$output" ]]; then
    echo "Refusing to resume or overwrite existing probe output: $output"
    exit 1
  fi
done
...
echo $! > "$PID_FILE"
echo "Started 5D interaction-risk probe."
