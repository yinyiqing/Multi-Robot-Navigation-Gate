...
if ! rosnode list >/dev/null 2>&1; then
  ...
  echo "Start cooperative detached multi-agent training first, or confirm it is still running."
  exit 1
fi

echo "Opening RViz for cooperative multi-agent training at $ROS_MASTER_URI"
