...
PID_FILE="$PROJECT_ROOT/.train_multi_stage2_2d_local_critic_from_2a_gentle_detached.pid"
LAUNCHFILE="multi_robot_scenario_multi_2.launch"
MODEL_NAME="TD3_velodyne_multi_v4_curriculum_stage2_2d_local_critic_from_2a_gentle"
...
log_file="$LOG_DIR/train_multi_stage2_2d_local_critic_from_2a_gentle_detached_${timestamp}.log"

if [[ -f "$PID_FILE" ]]; then
  old_pid="$(cat "$PID_FILE" 2>/dev/null || true)"
  if [[ -n "$old_pid" ]] && kill -0 "$old_pid" 2>/dev/null; then
    echo "A detached gentle stage2 2D local-critic training process is already running with PID $old_pid"
    exit 1
  fi
fi
...
setsid bash -lc "
  ...
  export DRL_MULTI_ACTOR_LR='${DRL_MULTI_ACTOR_LR:-0.000002}'
  export DRL_MULTI_CRITIC_LR='${DRL_MULTI_CRITIC_LR:-0.00008}'
  export DRL_MULTI_LOCAL_CRITIC_ACTOR_UPDATE_DELAY_STEPS='${DRL_MULTI_LOCAL_CRITIC_ACTOR_UPDATE_DELAY_STEPS:-25000}'
  export DRL_MULTI_TRAINING_VERSION='stage2-2d-local-critic-from-2a-gentle-v1'
...
echo "Detached gentle stage2 2D local-critic training started."
echo "PID: $(cat "$PID_FILE")"
echo "Agents: 2"
echo "Model: $MODEL_NAME"
echo "Actor warm start: $LOAD_MODEL_NAME"
echo "Critic: local neighborhood context, newly initialized"
echo "Actor update delay steps: ${DRL_MULTI_LOCAL_CRITIC_ACTOR_UPDATE_DELAY_STEPS:-25000}"
