...
case "$MODE" in
  20k)
    ...
    R2_ACTOR_SHA="67290450484c1fedd493fb029804b914438c5fb46cdb189ba8c642c3d98b2715"
    ;;
  10k)
    R2_ACTOR="$TD3_DIR/pytorch_models/capacity_wide_r2_s4_broad_n5_seed20260816_epoch_001_actor.pth"
    R2_ACTOR_SHA="ace910553931873a275d66e3a964fd2b4716d30b6c68c8dcb3e7af96e56783ee"
    ;;
  *)
    echo "Usage: $0 [20k|10k]" >&2
    exit 2
    ;;
esac
...
if [[ "${DRL_MULTI_DRY_RUN:-0}" == 1 ]]; then
  ...
  echo "Policies: 5A and R2-S4 $MODE"
...
setsid bash "$PROJECT_ROOT/scripts/run_g12_r2_n5_admission_worker.sh" \
  "$MODE" >>"$RUNNER_LOG" 2>&1 < /dev/null &
echo $! > "$PID_FILE"

echo "Started G12-R2 N5 paired admission."
echo "PID: $(cat "$PID_FILE")"
echo "Policies: 5A -> R2-S4 $MODE; 120 episodes each"
