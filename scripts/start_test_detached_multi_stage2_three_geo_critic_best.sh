...
MODEL_NAME="${DRL_MULTI_TEST_FILE_NAME:-TD3_velodyne_multi_v4_curriculum_stage2_to_3d2_geo_critic_from_3a_guarded_best}"
ROS_PORT="${DRL_MULTI_TEST_ROS_PORT:-11380}"
GAZEBO_PORT="${DRL_MULTI_TEST_GAZEBO_PORT:-11480}"
...
log_file="$LOG_DIR/test_multi_stage2_three_geo_critic_best_${SAFE_MODEL}_detached_${timestamp}.log"

if [[ -f "$PID_FILE" ]]; then
  old_pid="$(cat "$PID_FILE" 2>/dev/null || true)"
  if [[ -n "$old_pid" ]] && kill -0 "$old_pid" 2>/dev/null; then
    echo "A detached stage2 three-agent geometry-critic test process is already running with PID $old_pid"
    exit 1
  fi
fi
...
setsid bash -lc "
  ...
  export DRL_MULTI_TEST_STATE_PATH='./checkpoints/${SAFE_MODEL}_stage2_three_geo_critic_test_state.pt'
  export DRL_MULTI_TEST_STATS_PATH='./results/${SAFE_MODEL}_stage2_three_geo_critic_test.npy'
...
echo "Detached stage2 three-agent geometry-critic best test started."
