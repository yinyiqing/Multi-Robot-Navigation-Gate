...
if [[ ! -f "$PID_FILE" ]]; then
  echo "No managed G12-R2-S1 broad validation is running."
  exit 0
fi
...
if ! g12_r2_s1_ports_are_free "$ROS_PORT" "$GAZEBO_PORT"; then
  echo "S1 repair validation ROS/Gazebo ports are still occupied" >&2
  exit 1
fi
echo "Stopped G12-R2-S1 broad validation."
