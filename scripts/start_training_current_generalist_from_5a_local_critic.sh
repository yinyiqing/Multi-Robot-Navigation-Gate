...
MIN_REPLAY_SIZE="${DRL_MULTI_MIN_REPLAY_SIZE:-5000}"
DISCOUNT="${DRL_MULTI_DISCOUNT:-0.999}"
CRITIC_WARMUP_EXPL_NOISE="${DRL_MULTI_CRITIC_WARMUP_EXPL_NOISE:-0.08}"
EXPL_NOISE="${DRL_MULTI_EXPL_NOISE:-0.025}"
EXPL_MIN="${DRL_MULTI_EXPL_MIN:-0.012}"
...
[[ "$BATCH_SIZE" =~ ^[1-9][0-9]*$ ]] || {
  echo "DRL_MULTI_BATCH_SIZE must be a positive integer" >&2
  exit 2
}
[[ "$MIN_REPLAY_SIZE" =~ ^[0-9]+$ ]] || {
  echo "DRL_MULTI_MIN_REPLAY_SIZE must be a non-negative integer" >&2
  exit 2
}
...
setsid bash -lc "
  ...
  export DRL_MULTI_CRITIC_WARMUP_EXPL_NOISE='$CRITIC_WARMUP_EXPL_NOISE'
...
echo "Minimum replay size: $MIN_REPLAY_SIZE"
echo "Discount: $DISCOUNT"
echo "Critic warmup exploration noise: $CRITIC_WARMUP_EXPL_NOISE"
echo "Exploration noise: $EXPL_NOISE"
echo "Fixed physics step size: $FIXED_STEP_SIZE"
echo "Robot safe distance: $ROBOT_SAFE_DISTANCE"
