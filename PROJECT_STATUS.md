...
已开始英文投稿稿整理，当前稿件位于
[ICRA anonymous manuscript](paper/icra2027_template/main.tex)，内容与冻结的 G25 主检验、G26
探索性补充及当前证据边界一致，明确写出成功/安全收益、时间与交互调用代价，以及不外推到所有
重训单 Actor、真实机器人或任意车辆数量的限制。冻结图表已重新生成并保持原始输入/输出哈希不变。
...
G25 sealed 完成、归档和冻结统计后，按
[G26数量泛化与外部切换基线](experiments/03_保留专门化/02_论文主线/26_数量泛化与外部切换基线/README.md)
依次补充两项外部有效性证据：冻结系统在3车/7车上的零更新数量泛化，以及受IROS 2024
...
G25 sealed dry-run已通过：七种冻结方法均在Dense validation前2场完成统一入口、逐场落盘、
