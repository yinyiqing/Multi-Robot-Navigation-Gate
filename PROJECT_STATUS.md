...
- `N5/R2-10k`使用新的完整课程与连续Critic，同时改变训练协议，只回答更强训练流程能否
  替代旧策略路由，不能称为纯容量对照；
- G17/G18中F-A1相对R2B的结果进入主容量对照；
- [G20统一评测](experiments/03_保留专门化/02_论文主线/20_夜间最终统一评测/README.md)
  继续同场运行`N5/R2-10k/F-A1/N5+epoch17 recovery-oracle`，作为跨协议强基线审计。

当前不训练或修改任何Actor，不恢复R2D，也不因G20结果启动N5专用Gate。G20结果必须报告，
但不得把“更换课程与优化协议”和“只增加参数量”混成一个因果比较。
