...
PID_FILE="$PROJECT_ROOT/.train_multi_stage3_asym_three_5_joint_action_critic_midcheck.pid"
...
MODEL_NAME="${DRL_MULTI_TRAIN_FILE_NAME:-TD3_velodyne_multi_v4_curriculum_stage3_asym_three_5_joint_action_critic_midcheck_from_stage3_pair_cleanstart_v2_best}"
...
ROS_PORT="${DRL_MULTI_ROS_PORT:-12581}"
GAZEBO_PORT="${DRL_MULTI_GAZEBO_PORT:-12681}"

mkdir -p "$LOG_DIR"

timestamp="$(date +%Y%m%d_%H%M%S)"
log_file="$LOG_DIR/train_multi_${STAGE}_joint_action_critic_midcheck_detached_${timestamp}.log"

if [[ -f "$PID_FILE" ]]; then
  old_pid="$(cat "$PID_FILE" 2>/dev/null || true)"
  if [[ -n "$old_pid" ]] && kill -0 "$old_pid" 2>/dev/null; then
    echo "A detached joint-action critic midcheck training process is already running with PID $old_pid"
    exit 1
  fi
fi
...
if [[ -n "$existing_pid" ]]; then
  ...
  echo "Please stop it before starting joint-action critic midcheck."
  exit 1
fi
...
setsid bash -lc "
  ...
  export DRL_MULTI_TRAINING_VERSION='stage3-asym-three-5-joint-action-critic-midcheck-v1'
...
echo "Detached joint-action critic midcheck training started."
...
echo "Purpose: target-fix mid-length validation"
