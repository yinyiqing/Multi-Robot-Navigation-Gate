...
减少`239.2 +/- 30.1`次。主seed和`4/4`复核seed通过，超过预注册的`3/4`要求，
因此G11-A1判定为通过。

五个seed的T1最优点均为epoch 2。最优checkpoint的train/validation F1均值分别为
`0.85922/0.85293`；继续训练到epoch 40时validation F1均值降至`0.81258`，AP均值
降至`0.89407`。这是明确的后期过拟合，因此后续沿用早停checkpoint，不追加epoch扫描。
