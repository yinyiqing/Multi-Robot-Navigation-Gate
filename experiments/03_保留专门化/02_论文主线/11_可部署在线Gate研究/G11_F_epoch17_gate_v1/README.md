...
该补充pilot已完成`100/100`场并通过场景顺序、终止计数和结果形状审计。合并结果为：

| 策略 | Full success | Agent success | Collision | Timeout | 平均步数 |
| --- | ---: | ---: | ---: | ---: | ---: |
| 5A | `0.640` | `0.896` | `0.104` | `0.000` | `21.91` |
| F-A1 | `0.710` | `0.912` | `0.088` | `0.000` | `33.41` |
| F-B2 | `0.680` | `0.914` | `0.080` | `0.030` | `44.87` |
...
`0.458/0.542`；两个0-edge层分别为`1.000/1.000`和`0.962/1.000`。F-A1在两个0-edge
层仍有约`0.65/0.47`的避障Actor占比，平均步数分别为`29.88/26.35`，而R2-10k为
...
论文公平容量baseline排名或Gate取舍。流程匹配的R2B后续在Actor解冻后训练坍塌；项目
接受退化作为对照结果，因此冻结真正更新过的R2B-30k作为流程/预算匹配大Actor
baseline，不使用冻结期自动best。该对照必须同时报告训练坍塌。
