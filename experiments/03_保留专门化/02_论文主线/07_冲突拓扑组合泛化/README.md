...
状态：`frozen evidence`。旧G3/G4未通过最终准入，但其Oracle分层和exact-edge-2方向性
收益是新主线的设计依据；当前执行协议见[父目录README](../README.md)。

本目录保存第一版Gate的组合泛化证据和失败边界，不再作为当前执行台账。
...
200场结果、分层统计和未通过决策见
[`G3_200场准入`](G3_200场准入/README.md)。G4协议和锚点恢复失败见
[`G4_G2B_v2多次Rollout标签`](G4_G2B_v2多次Rollout标签/README.md)。当前禁止直接
