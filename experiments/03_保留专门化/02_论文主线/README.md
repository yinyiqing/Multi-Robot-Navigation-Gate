...
参数量与训练流程分开回答：R2B严格匹配5A起点、五车数据、预算和优化流程，是主表中的
纯容量对照；N5/R2-10k使用新课程和连续Critic，是跨协议强基线，不能解释成“只把参数量
翻倍”。当前[20_夜间最终统一评测](20_夜间最终统一评测/README.md)继续同场比较
`N5-only / R2-10k / F-A1 / N5+epoch17 recovery-oracle`，用于量化强训练流程边界和
互补性变化，不用于把主方法切换到N5组合。

当前不再训练Actor、不恢复R2D、不启动N5专用Gate，也不得用不同seed或冲突比例拼接主表。
