...
4. 在完整1000场dense validation上确认候选独立超过5A/5D，并且没有系统性超时。
5. 补测新Actor的0-edge/standard validation能力，确认它与5A是否真的互补；如果新Actor全面优于5A，则不强行训练无意义的Gate。
6. 只有两个独立Actor存在稳定互补性时，才冻结它们并训练Gate。
7. Gate使用可部署的本机感知做状态选择，不读取场景名、dense标签或其他机器人真值。
8. Actor、Gate和所有阈值冻结后，最后一次运行sealed test。
