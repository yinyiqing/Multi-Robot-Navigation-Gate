...
状态：`registered / queued`。登记日期：`2026-08-15`。

## 目的

在历史dense256完全相同的场景、seed和物理协议上补跑当前流程匹配容量baseline
`R2B-best`，判断它在高密度多冲突压力集上的表现。该实验只做评测，不更新任何Actor、
Gate或阈值，不读取sealed test。
...
完成后报告overall和`0/1/2/3+`分层，并与历史同场5A、B2、2m规则、R2-10k比较。
实时日志：`logs/active/g18-dense256-r2b/`；完成后归档到
`logs/archive/validation/g18_dense256_r2b/`。
