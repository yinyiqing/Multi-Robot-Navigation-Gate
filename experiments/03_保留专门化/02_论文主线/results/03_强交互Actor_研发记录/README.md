...
| `20260801_简化TD3全程动作覆盖实验D` | ready | 保持C所有参数，只将线速度均衡覆盖延长到Actor解冻前的21000 samples |

v6 epoch-11的200场复核已完成：收益真实，但没有通过独立Dense Actor的timeout和效率验收。v8确认不能直接放开Critic；v9的单边危险加速上限虽然阻止两类退化，却没有产生学习增益。实验A进一步证明高学习率下随机Critic会立即破坏5A。实验B虽然修正了reward和训练尺度，但Critic仍因危险状态的中低速样本不足而错误偏好全速。实验C证明均衡动作覆盖可以修正排序，但覆盖停止后错误复发。当前只运行实验D验证完整warmup覆盖；审计通过前不解冻Actor。
