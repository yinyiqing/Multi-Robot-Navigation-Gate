...
后续 replay 距离分层审计进一步确认：无可见邻居状态占 `40.02%`，`1.2--2.0 m`、`0.8--1.2 m`、`<=0.8 m` 分别占 `28.25%/10.28%/1.91%`。Actor 在无邻居状态上的线速度变化最大（raw action `+0.184`）；在 `<=0.8 m` 危险状态仍增加 `+0.125`，且 Critic 对这些明显变化有 `88.79%` 判断新动作更优。这说明强Actor被大量非交互状态更新，同时危险样本上的价值方向错误。完整数据见 `replay_interaction_audit.json`。
