...
| 起点区域 | `[-4.5,4.5]^2` | `[-2.0,2.0]^2` | `[-1.75,1.75]^2` | `[-1.65,1.65]^2` |
| 名义采样面积 | `81 m^2` | `16 m^2` | `12.25 m^2` | `10.89 m^2` |
| 名义机器人密度 | `5/81 = 0.062 robot/m^2` | `5/16 = 0.313 robot/m^2` | `5/12.25 = 0.408 robot/m^2` | `5/10.89 = 0.459 robot/m^2` |
| 相对 standard 密度 | `1.00x` | `5.06x` | `6.61x` | `7.44x` |
| 起点最小车距 | `0.95 m` | 配置 `0.9 m`，实际至少 `1.2 m` | 共享逻辑至少 `1.2 m` | 共享逻辑至少 `1.2 m` |
| goal x/y offset | x `[-2.2,2.2]`，y `[-2.4,2.4]` | x/y `[-1.2,1.2]` | x/y `[-1.8,1.8]` | x/y `[-2.0,2.0]` |
| 单车目标距离 | 约 `0.8-3.26 m` | 配置 `0.6-1.70 m` | 配置 `0.8-2.20 m` | 配置 `0.9-2.30 m` |
| 目标间距 | `0.85 m`，可放宽至 `0.45 m` | `0.8 m`，可放宽至 `0.48 m` | `0.8 m`，可放宽至 `0.48 m` | `0.8 m`，可放宽至 `0.48 m` |
| 目标与任意机器人间距 | `0.75 m` | `0.8 m` | `0.8 m` | `0.8 m` |
...
- tight1/tight2 由参数化脚本 `scripts/start_test_detached_dense5_random_5d.sh` 启动，配置已可恢复；历史日志本身仍未内嵌完整 manifest，正式论文实验应在重跑时写入 scenario manifest。
