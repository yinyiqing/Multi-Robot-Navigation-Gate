...
状态：`5D 冻结为弱交互 Actor；strong-interaction-v1 数据已冻结，时序强交互 Actor pilot 已就绪`。
...
- `pi_I`：复制 5D 权重初始化、随后独立训练，并读取局部观测历史的强交互 Actor。
...
弱交互与紧迫冲突需要不同的行为偏好，但执行阶段不能读取离线风险标签。假设时序强交互 Actor 能形成与 5D 互补的行为，Gate 能仅凭本车局部历史在状态级选择更合适的 Actor。
...
- 保留原始 5D 作为独立弱交互 Actor；强交互 Actor 复制 5D 权重 warm-start，复制后的完整主干和新增时序模块都参与训练。
- 强交互 Actor 读取最近 `H=8` 帧本地 24 维观测；新增时序分支零初始化，因此训练起点严格等价于 5D。
- 训练视图为 `strong_interaction_v1`：`512 deep + 128 close + 128 margin`；validation 为 `60 + 40 + 40`。
- deep 是主要优化目标，close 是边界集，margin 用于约束过度干预。
- Actor 和后续 Gate 都不读取离线 risk、standard 或 dense 标签。
- reward 保留 `0.8 self + 0.2` 距离加权邻居项，并加入训练期近距离与停滞约束。
- pilot 准入条件：deep full success 相对同协议 5D 至少 `+15 points` 且碰撞下降；close/margin 下降不超过 `5/3 points`。
...
当前仍处于 `D4`。5D 已冻结为弱交互 Actor；`strong_interaction_v1` 时序强交互 Actor pilot 已就绪。先运行一个 seed 的 40000-agent-sample 受控 pilot，只有达到预设 deep/close/margin 准入条件才扩展到三 seed。Gate 仍需等待 D5 互补性审计通过。
...
2. 保留可靠的弱交互导航能力，并训练面向紧迫冲突的时序强交互 Actor，通过配对评估验证互补性。
