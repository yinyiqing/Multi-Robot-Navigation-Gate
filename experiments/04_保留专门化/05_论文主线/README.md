...
状态：`D4 timeout-fix v3 已完成，epoch 2 有正向信号，待扩大 validation 复核`。
...
- v3 在 100 场 validation 上由 5D 的 `0.870/0.580` 提高到 epoch 2 的 `0.882/0.620`（agent/full success），但 epoch 3 回落；先扩大 validation 复核，不读取 test。
